...
import { Tour, TourCategory, CATEGORY_LABELS, LANGUAGE_LABELS } from '@/types';
...
const filters: { key: TourCategory | 'all'; icon: string }[] = [
  { key: 'all', icon: '✨' },
  { key: 'cultural', icon: '🏛️' },
  { key: 'nature', icon: '🌿' },
  { key: 'boat', icon: '⛵' },
...
function getFilterLabel(key: string): string {
  if (key === 'all') return 'Tüm Turlar';
  return CATEGORY_LABELS[key as TourCategory]?.tr || key;
}
...
export default function TourGrid({ tours }: TourGridProps) {
  ...
  // Collect all unique languages across tours
  const allLanguages = useMemo(() => {
    const langSet = new Set<string>();
    tours.forEach((t) => t.languages?.forEach((l) => langSet.add(l)));
  ...
  const filtered = useMemo(() => {
    ...
    if (pRange && priceFilter !== 'all') {
      result = result.filter((t) => t.price >= pRange.min && t.price <= pRange.max);
    }
    // Language filter
    if (langFilter !== 'all') {
      result = result.filter((t) => t.languages?.includes(langFilter));
    }
    return result;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-2xl sm:text-4xl font-bold text-neutral-900 mb-3 sm:mb-4">
            Kuşadası&apos;nın En Sevilen Turları
          </h2>
          <p className="text-neutral-500 max-w-xl mx-auto text-base sm:text-lg">
            Profesyonel rehberler eşliğinde, en yüksek puanlı tur deneyimlerinden birini seçin.
          </p>
        </motion.div>

        {/* Category Filter bar */}
        <motion.div
          className="flex items-center gap-2 mb-4 overflow-x-auto pb-2 sm:pb-0 sm:flex-wrap sm:justify-center scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0"
        ...
        <div className="flex justify-center mb-6 sm:mb-8">
          ...
          >
            ...
            <svg className={cn('w-4 h-4 transition-transform duration-300', filtersExpanded && 'rotate-180')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              <div className="bg-neutral-50 rounded-2xl p-4 sm:p-6 mb-8 border border-neutral-100">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  {/* Price filter */}
                  <div>
                    <label className="block text-sm font-semibold text-neutral-700 mb-2.5">💰 Fiyat Aralığı</label>
                    <div className="flex flex-wrap gap-2">
                      {priceRanges.map((p) => (
                        <button
                          ...
                          className={cn(
                            'px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-200 border min-h-[40px]',
                            priceFilter === p.key
                              ? 'bg-ice-500 text-white border-ice-500'
                              : 'bg-white text-neutral-600 border-neutral-200 hover:border-ice-300'
                          )}
                        >
                          {p.label}
                        </button>
                  ...
                  {/* Language filter */}
                  <div>
                    <label className="block text-sm font-semibold text-neutral-700 mb-2.5">🗣️ Tur Dili</label>
                    <div className="flex flex-wrap gap-2">
                      <button
                ...
                {hasActiveAdvancedFilters && (
                  <div className="mt-4 pt-3 border-t border-neutral-200">
                    <button
                      ...
                      className="text-sm text-ice-600 hover:text-ice-700 font-medium min-h-[40px]"
                    >
                      ✕ Filtreleri Temizle
                    </button>
                  </div>
        ...
        {filtered.length === 0 && (
          ...
          >
            ...
            <p className="text-neutral-600 text-lg font-medium mb-2">Bu filtrelere uygun tur bulunamadı.</p>
            <p className="text-neutral-400 text-sm mb-6">Filtreleri değiştirerek tekrar deneyin.</p>
            <button
              onClick={() => { setActiveFilter('all'); setPriceFilter('all'); setLangFilter('all'); }}
        ...
        {filtered.length > 0 && (
          ...
          >
            <a href="#contact" className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full border-2 border-ice-200 text-ice-600 font-semibold hover:bg-ice-50 transition-colors text-sm min-h-[48px]">
