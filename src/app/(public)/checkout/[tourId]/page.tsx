...
export default function CheckoutPage({ params }: { params: Promise<{ tourId: string }> }) {
  ...
  const [form, setForm] = useState({
    ...
    customerEmail: '',
    customerPhone: '',
    language: searchParams.get('lang') || availableLanguages[0] || 'en',
    ...
    return (
      <main className="min-h-screen bg-neutral-50 pt-20 flex items-center justify-center">
        <div className="text-center">
          <p className="text-neutral-500 mb-4">Tur bulunamadı.</p>
  ...
  const validate = () => {
    ...
    if (!form.customerName.trim()) e.customerName = 'İsim zorunludur';
    if (!form.customerEmail.trim() || !/\S+@\S+\.\S+/.test(form.customerEmail)) e.customerEmail = 'Geçerli e-posta giriniz';
    if (!form.customerPhone.trim() || form.customerPhone.replace(/\D/g, '').length < 10) e.customerPhone = 'Geçerli telefon giriniz';
    if (form.count < 1) e.count = 'En az 1 kişi';
    if (form.count > capacity.available) e.count = `Maksimum ${capacity.available} kişi`;
    setErrors(e);
    ...
    <main className="min-h-screen bg-neutral-50 pt-16 lg:pt-20 pb-24 lg:pb-12">
      ...
      <div className="bg-white border-b border-neutral-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm text-neutral-400" aria-label="Checkout breadcrumb">
            <Link href="/" className="hover:text-ice-500 transition-colors">Ana Sayfa</Link>
            <span>/</span>
            <Link href={`/tours/${tour.id}`} className="hover:text-ice-500 transition-colors">{tour.title}</Link>
            <span>/</span>
            <span className="text-neutral-700 font-medium">Rezervasyon</span>
          </nav>
        </div>
      </div>
        ...
        <div className="lg:grid lg:grid-cols-5 lg:gap-8">
          ...
          <motion.div className="lg:col-span-3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-5 sm:p-7">
              <h1 className="text-xl sm:text-2xl font-bold text-neutral-900 mb-6">📋 Rezervasyon Bilgileri</h1>

              <div className="space-y-5">
                {/* Name */}
                <div>
                  <label htmlFor="checkout-name" className="block text-sm font-medium text-neutral-700 mb-1.5">İsim Soyisim *</label>
                  <input id="checkout-name" type="text" value={form.customerName}
                    ...
                    placeholder="Adınız ve Soyadınız" aria-label="İsim Soyisim" />
                  {errors.customerName && <p className="text-red-500 text-xs mt-1">{errors.customerName}</p>}
                </div>

                {/* Email */}
                <div>
                  <label htmlFor="checkout-email" className="block text-sm font-medium text-neutral-700 mb-1.5">E-posta *</label>
                  <input id="checkout-email" type="email" value={form.customerEmail}
                    ...
                    placeholder="ornek@email.com" aria-label="E-posta adresi" />
                  {errors.customerEmail && <p className="text-red-500 text-xs mt-1">{errors.customerEmail}</p>}
                </div>

                {/* Phone */}
                <div>
                  <label htmlFor="checkout-phone" className="block text-sm font-medium text-neutral-700 mb-1.5">Telefon (WhatsApp) *</label>
                  <div className="relative">
                    ...
                    <input id="checkout-phone" type="tel" value={form.customerPhone}
                      ...
                      placeholder="+90 555 123 4567" aria-label="Telefon numarası" />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="checkout-lang" className="block text-sm font-medium text-neutral-700 mb-1.5">Rehber Dili</label>
                    <select id="checkout-lang" value={form.language}
                      onChange={e => setForm(p => ({ ...p, language: e.target.value }))}
                      className="w-full px-4 py-3 rounded-xl border border-neutral-200 text-sm outline-none focus:ring-2 focus:ring-ice-500 bg-white" aria-label="Rehber dili seçimi">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-neutral-700 mb-1.5">Katılımcı Sayısı</label>
                    <div className="flex items-center gap-0 border border-neutral-200 rounded-xl overflow-hidden">
                      <button onClick={() => setForm(p => ({ ...p, count: Math.max(1, p.count - 1) }))}
                        className="px-4 py-3 text-lg font-bold text-neutral-500 hover:bg-neutral-50 transition-colors min-w-[48px] min-h-[48px]" aria-label="Katılımcı azalt">−</button>
                      ...
                      <button onClick={() => setForm(p => ({ ...p, count: Math.min(capacity.available, p.count + 1) }))}
                        className="px-4 py-3 text-lg font-bold text-neutral-500 hover:bg-neutral-50 transition-colors min-w-[48px] min-h-[48px]" aria-label="Katılımcı artır">+</button>
                    </div>
                ...
                {/* Special Notes */}
                <div>
                  <label htmlFor="checkout-notes" className="block text-sm font-medium text-neutral-700 mb-1.5">📝 Özel Not <span className="text-neutral-400 font-normal">(opsiyonel)</span></label>
                  <textarea id="checkout-notes" value={form.specialNotes}
                    ...
                    placeholder="Alerji, engelli durumu, özel diyet, çocuk koltuğu vb." aria-label="Özel notlar" />
                </div>
              </div>
          ...
          <motion.div className="lg:col-span-2 mt-6 lg:mt-0" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-5 sm:p-6 lg:sticky lg:top-24">
              <h2 className="text-lg font-bold text-neutral-900 mb-4">📦 Tur Özeti</h2>
              ...
              <h3 className="font-bold text-neutral-900 mb-3">{tour.title}</h3>

              <div className="space-y-2.5 text-sm mb-5">
                <div className="flex items-center justify-between">
                  <span className="text-neutral-500">📅 Tarih</span>
                  <span className="font-medium text-neutral-700">{new Date().toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-neutral-700">{form.count} kişi</span>
                </div>
                <div className="flex items-center justify-between">
              ...
              <div className="border-t border-neutral-100 pt-4 space-y-2 text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-neutral-500">Birim fiyat</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-neutral-500">Hizmet bedeli</span>
                  <span className="text-emerald-600 font-medium">Dahil</span>
                </div>
              </div>

              <div className="border-t border-neutral-100 pt-4 mt-4">
                <div className="flex items-center justify-between">
                  <span className="text-lg font-bold text-neutral-900">Toplam</span>
