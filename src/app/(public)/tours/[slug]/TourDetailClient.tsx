...
import { Tour, CATEGORY_LABELS, LANGUAGE_LABELS } from '@/types';
...
export default function TourDetailClient({ tour, relatedTours = [] }: TourDetailClientProps) {
  ...
  const categoryLabel = CATEGORY_LABELS[tour.category]?.tr || tour.category;
      ...
      <motion.div className="bg-white border-b border-neutral-100" initial="hidden" animate="visible" variants={fadeUp} custom={0}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm text-neutral-400">
            ...
            <span>/</span>
            <Link href="/#tours" className="hover:text-ice-500 transition-colors">Turlar</Link>
            <span>/</span>
            <span className="text-neutral-700 font-medium truncate">{tour.title}</span>
          </nav>
        </div>
      </motion.div>
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <motion.div className="relative aspect-[16/10] sm:aspect-[16/9] rounded-2xl overflow-hidden shadow-xl" initial="hidden" animate="visible" variants={fadeUp} custom={1}>
              ...
              {!tour.isOpen && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                  <span className="px-6 py-3 bg-red-600 text-white font-bold rounded-full text-lg shadow-lg">Satış Kapalı</span>
                </div>
              )}
            </motion.div>
            ...
            <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={2}>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-neutral-900 mb-3 leading-tight">{tour.title}</h1>
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="text-neutral-400">({tour.reviewCount} değerlendirme)</span>
              </div>
              {/* Languages */}
              <div className="flex flex-wrap items-center gap-2 mb-6">
                <span className="text-sm font-medium text-neutral-600 mr-1">Diller:</span>
              ...
              <p className="text-neutral-600 leading-relaxed text-base sm:text-lg">{tour.description}</p>
            </motion.div>
                  ...
                  {descExpanded ? (
                    ...
                    <motion.div key="preview" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                      <div className="px-5 pb-5 sm:px-6 sm:pb-6 pt-0 relative">
                        <p className="text-neutral-500 leading-relaxed text-sm line-clamp-3">{tour.fullDescription.split('\n\n')[0]}</p>
            ...
            <motion.div className="lg:sticky lg:top-24" initial="hidden" animate="visible" variants={fadeUp} custom={2}>
              <div className="bg-white rounded-2xl p-5 sm:p-6 border border-neutral-100 shadow-xl shadow-neutral-200/40">
                <div className="flex items-baseline gap-2 mb-1">
                  ...
                  <span className="text-neutral-400 text-sm">/ kişi başı</span>
                </div>
                ...
                <motion.button whileHover={!isSoldOut && tour.isOpen ? { scale: 1.02 } : {}} whileTap={!isSoldOut && tour.isOpen ? { scale: 0.98 } : {}}
                  ...
                  disabled={isSoldOut || !tour.isOpen}
                  ...
                  aria-label={isSoldOut ? 'Yer kalmadı' : 'Şimdi rezerve et'}>
                  {isSoldOut ? '🚫 Yer Kalmadı' : !tour.isOpen ? '🔴 Satış Kapalı' : 'Şimdi Rezerve Et'}
                </motion.button>
                <p className="text-center text-xs text-neutral-400 mt-3">Ücretsiz iptal • Anında onay</p>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-neutral-200 px-4 py-3 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <span className="text-neutral-400 text-xs ml-1">/ kişi</span>
          </div>
          <motion.button whileTap={!isSoldOut && tour.isOpen ? { scale: 0.96 } : {}}
            ...
            aria-label={isSoldOut ? 'Yer kalmadı' : 'Rezerve et'}>
            {isSoldOut ? 'Yer Kalmadı' : !tour.isOpen ? 'Kapalı' : 'Rezerve Et'}
          </motion.button>
        </div>
      </div>
    </main>
